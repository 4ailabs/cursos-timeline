...
## 6. El desenfoque como error de predicción

TAME se integra con la **inferencia activa**, que describe a los sistemas vivos como minimizadores de error de predicción (Friston et al., 2015). En el ojo esto es literal: **el desenfoque es una señal de error**, y su *signo* es la información que indica en qué dirección corregir. El sistema visual sintoniza su óptica al entorno que **predice** habitar; una infancia de visión cercana lo lleva a prepararse para un mundo cercano. La miopía es esa predicción cumplida en la longitud del ojo. Este marco une los tres niveles previos: el error de predicción es la señal del circuito (sección 2), procesada por el lazo de control (sección 3), al servicio de la meta del agente (sección 5).
...
*Las referencias 3–8 deben verificarse en su forma final (volumen, página) con los coautores oftalmólogos; algunas citas combinan revisión y fuente primaria.*

---

*Borrador v3. Añade el circuito neurobiológico (retina–EPR–coroides–esclera, ipRGC/dopamina, EGR1/glucagón/ácido retinoico, mecanismo de la atropina) y la arquitectura cibernética (modelos de control de la emetropización), sobre los que descansa la lectura agencial (TAME). Listo para revisión de los coautores; siguiente paso: versión LaTeX de calidad de revista.*
