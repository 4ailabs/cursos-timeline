...
**Manual de Regulación Bioeléctrica** — Dr. Miguel Ojeda Rios · Instituto Centrobioenergetica
...
1. **Cuántos nodos hay** — uno, dos o varios.
2. **Cómo se reparten en el cuerpo** — concentrados en un solo lugar, o en tres o cinco órganos distintos.
3. **Con qué corresponden en la historia clínica** — qué molestias de la persona coinciden con esos nodos.
...
1. **El organismo tiene un mapa eléctrico propio**, con polaridades definidas: el centro positivo y las extremidades negativas.
2. **El voltaje instruye la regeneración.** Sin el patrón de voltaje adecuado no ocurre ni la regeneración ni la cicatrización.
3. **La polaridad influye sobre el desarrollo bacteriano.** Con plata y voltaje positivo, más un campo de polo negativo, se frenó el desarrollo de bacterias en cultivo. Es el único dato disponible sobre el efecto de un campo eléctrico o magnético sobre microorganismos. El efecto es indirecto: el campo modifica el voltaje de la zona y a partir de ahí cambia el comportamiento del biofilm.
4. **El hueso es piezoeléctrico.** Genera corriente al recibir carga mecánica, y se deforma al aplicarle corriente.
...
```
        ┌──────────────────────────────┐
        │                              ▼
   VOLTAJE DE MEMBRANA        Qué canales iónicos se abren
        ▲                              │
        │                              ▼
        │                        Flujo de iones
        │                              │
        │                              ▼
        └────── pH · resistividad · estado de oxidación-reducción
```
...
1. **La meta.** Lo que ese órgano consigue cuando trabaja bien.
2. **La capacidad de actuar.** Depende de sus recursos, de su integridad estructural y de las señales que le llegan del entorno.
3. **El punto de ajuste.** El valor de calibración desde el que opera, aquello que para ese agente cuenta como normal.
...
K = log₁₀ ( τ ciego ÷ τ agente )

τ agente es lo que tarda el sistema. τ ciego es lo que tardaría el azar. El cociente dice cuántas veces más rápido es el sistema, y el logaritmo convierte ese número en órdenes de magnitud, que es la única forma de comparar sistemas que difieren por factores enormes.
...
| **21** | 10²¹ veces | La adaptación de una planaria a un tóxico nuevo |
...
Σ = K del conjunto − (la suma de las K de sus partes)
...
Los dos se mantienen juntos, en posición de atracción. Al separarlos el gradiente se aplana y el efecto se pierde.
...
PRUEBA DE POLARIDAD DEL CONJUNTO MAGNÉTICO
  ...
  2 · Se desplaza el imán pequeño hacia el BORDE.
        El comportamiento se invierte, lo que confirma
        que los dos polos están en la misma cara.

  3 · El polo del CENTRO es el que se coloca contra la piel
        en el nodo que se está rastreando.
```
...
## 12 · El reflejo del iliopsoas y el acortamiento de la extremidad
...
1 · EXPLICAR EL PROCEDIMIENTO
    Se informa que consiste en una evaluación de simetría y una
    colocación de imanes, indolora y no invasiva.
...
4 · LIBERAR EL ILIOPSOAS
    Ambas extremidades inferiores se traccionan con suavidad para
    relajar la articulación pélvica, y después se movilizan hacia
    los lados para relajar el piso pélvico.

5 · CUBRIR
    Se cubre a la persona con una manta, que relaja el cuerpo y
    retira los mecanismos de defensa.
```

### La posición de medición se repite igual en cada colocación

```
POSICIÓN DE MEDICIÓN

    Los pies quedan por fuera del borde distal de la camilla,
    con los talones libres. De preferencia con calzado.

    Antes de medir, unas respiraciones profundas.

    Ambas extremidades se elevan por los tobillos unos 30 GRADOS,
    alineadas con el eje del cuerpo.

    Los brazos permanecen relajados. La punta del pie queda
    alineada con la rodilla; los pies pueden abrirse ligeramente.

    SE LEVANTA Y SE COMPARA. No se tracciona en ese momento
    y no se corrige nada.
```
...
|---|---|
| **Los dos talones parejos** | Se pasa directamente al rastreo por zonas |
| **Un talón más corto que el otro** | Se establece primero la línea base, con el eje de acidosis |
...
2 · El potasio del interior sale al espacio extracelular.
    La concentración pasa de alrededor de 4 mmol
    a valores de 20 a 40 mmol en la zona.

3 · Ese exceso de potasio despolariza a las células vecinas.

4 · La despolarización cierra las uniones comunicantes,
    y el grupo de células queda desconectado de la red.
```
...
1. **Movimientos oculares rápidos**, como los del sueño.
2. **Somnolencia o relajación profunda.** La persona se duerme con frecuencia.
3. **Cambios en la voz**, que se arrastra o baja de volumen.
...
```
1 · LA ZONA
    Se elige la siguiente zona del eje que se está trabajando,
    en el orden que ese eje establece.

2 · COLOCAR Y MEDIR
    Polo negativo, cara negativa a la piel, sobre la zona.
    Con el conjunto magnético, la pieza polar negativa contra
    la piel. Medir la simetría.

      Acorta            → zona confirmada. El imán queda fijo.
      Sin cambio        → se desplaza el imán dentro de la zona
                          y se vuelve a medir.
      Sin cambio en
      toda la zona      → se pasa a la zona siguiente.
...
|---|---|---|
| **Riñón** | Efector iónico. Es el único órgano que regula los iones de todo el organismo, y un cambio de 1 mmol produce desequilibrio bioeléctrico en tejidos distantes | Establecido |
| **Suprarrenal** | Efector neuroendocrino | Propuesto, en investigación |
| **Hígado** | Efector metabólico, por vía vagal | Propuesto, en investigación |
...
```
    ┌───────┬───────┐
    │   −   │   +   │
    ├───────┼───────┤     Cuatro imanes iguales entre sí,
    │   +   │   −   │     en cuadro de dos por dos,
    └───────┴───────┘     con las polaridades alternadas.
```
...
VERIFICACIÓN DE RETIRO
    ...
    Cumplido el tiempo, se retira ÚNICAMENTE el polo positivo
    y se mide.

      Talones parejos  → la zona se resolvió. Se retira también
                         el negativo y la sesión termina.
      Vuelve a acortar → se recoloca el positivo y se prolonga.
                         Se repite hasta que, al retirarlo,
                         los talones queden parejos.
```
...
1. **Dolor agudo intenso**, en particular acompañado de vómito.
2. **Dolor que despierta por la noche** de forma persistente.
3. **Alteración de los esfínteres.**
4. **Inicio en mayores de 50 años** atribuido a una causa mecánica.
5. **Pérdida de peso sin explicación.**
6. **Fiebre acompañada de dolor.**
7. **Déficit neurológico progresivo.**
8. **Cáncer activo o tratamiento oncológico en curso**, por posibilidad de compresión.
9. **Dolor visceral agudo.**
...
1. **Sangrado activo.**
2. **Dolor agudo masivo.**
3. **Crisis convulsiva en curso.**
4. **Infección aguda con fiebre.**
...
## 22 · La ficha y el mapa corporal
...
## 23 · Reglas de conducción del rastreo
...
A la persona le duele, el operador rastrea la zona entera y la medición no cambia. O el nodo empareja los talones, y a la semana siguiente la persona vuelve igual. Cada uno de esos casos tiene una explicación y una salida.
